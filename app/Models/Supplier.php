...
class Supplier extends Model
{
    ...
    protected static function booted(): void
    {
        ...
        static::created(fn (self $supplier) => $supplier->logActivity('created', ['title' => $supplier->name]));

        static::updated(function (self $supplier): void {
            $fields = array_keys(Arr::except($supplier->getChanges(), ['updated_at']));

            if ($fields !== []) {
                $supplier->logActivity('updated', ['fields' => $fields]);
            }
        });

        // Products of a private supplier can't stay public.
        static::updated(function (self $supplier): void {
            if ($supplier->wasChanged('visibility') && ! $supplier->isPublic()) {
                $supplier->products()
    ...
    /**
     * Whether another group has products for this supplier — then it
     * must stay public.
     */
