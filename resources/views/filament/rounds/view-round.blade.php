...
<x-filament-panels::page>
    ...
    {{-- Benachrichtigungs-Entwürfe --}}
    @if ($round->notificationDrafts->isNotEmpty())
        <x-filament::section heading="Benachrichtigungs-Entwürfe" :collapsible="true" :collapsed="true">
            <div class="space-y-3">
                @foreach ($round->notificationDrafts as $draft)
                    <div class="rounded-lg border border-gray-200 dark:border-white/10 p-3">
                        ...
                        <pre class="mt-2 whitespace-pre-wrap text-xs text-gray-600 dark:text-gray-400 max-h-48 overflow-auto">{{ $draft->body }}</pre>
                    </div>
                @endforeach
            </div>
        </x-filament::section>
    ...
    @if ($round->activities->isNotEmpty())
        <x-filament::section heading="Aktivitäten" :collapsible="true" :collapsed="true">
            <ul class="space-y-2 text-sm">
                @foreach ($round->activities()->limit(20)->get() as $a)
                    <li class="flex items-baseline gap-3">
                        <span class="text-xs text-gray-400 tabular-nums">{{ $a->created_at->format('d.m. H:i') }}</span>
                        <span class="text-gray-700 dark:text-gray-200">{{ $a->user?->first_name ?? 'System' }} · {{ $a->describe() }}</span>
                    </li>
                @endforeach
            </ul>
        </x-filament::section>
